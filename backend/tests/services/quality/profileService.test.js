...
function mockInsertRows(client, { profileRow, versionRow }) {
  client.query.mockImplementation((sql) => {
    const statement = String(sql);
    if (statement.includes('INSERT INTO quality_profiles')) {
      return Promise.resolve({ rows: [profileRow] });
    }
...
describe('profileService', () => {
  ...
  describe('createProfile', () => {
    it('creates a profile with immutable version 1 inside one transaction', async () => {
      const client = makeClient();
      db.connect.mockResolvedValue(client);
      mockInsertRows(client, {
        profileRow: { id: 'profile-1', user_id: 'user-1', name: 'Breakout' },
      ...
      expect(versionInsert[1][3]).toBe(config);

      const pointerUpdate = findCall(client.query, 'UPDATE quality_profiles');
      expect(pointerUpdate[1]).toEqual(['version-1', 'profile-1']);

      expect(client.release).toHaveBeenCalled();
    });

    it('seeds the canonical configuration when none is provided', async () => {
      const client = makeClient();
      db.connect.mockResolvedValue(client);
      mockInsertRows(client, {
        profileRow: { id: 'profile-1', user_id: 'user-1', name: 'Canonical BO' },
        versionRow: { id: 'version-1', profile_id: 'profile-1', version_number: 1 }
      });

      const created = await profileService.createProfile('user-1', { name: 'Canonical BO' });
      expect(created.id).toBe('profile-1');
    ...
    it('rejects a blank name and an invalid configuration before touching the database', async () => {
      await expect(profileService.createProfile('user-1', { name: '   ' }))
    ...
    it('rolls back and rethrows when a statement fails', async () => {
      ...
      await expect(profileService.createProfile('user-1', { name: 'Breakout' }))
  ...
  describe('createVersion', () => {
    it('creates the next immutable version and repoints current_version_id', async () => {
      ...
      const config = getCanonicalBOConfig();
      const version = await profileService.createVersion('profile-1', 'user-1', config);
      ...
      expect(insert[1]).toEqual(['profile-1', 2, 1, config]);
    });

    it('returns null when the profile does not exist or is not owned by the user', async () => {
      db.query.mockResolvedValue({ rows: [] });
      const version = await profileService.createVersion('profile-1', 'user-1', getCanonicalBOConfig());
  ...
  describe('ensureCanonicalBO', () => {
    it('creates the Canonical BO profile when the user has none', async () => {
      db.query.mockResolvedValue({ rows: [] });
      ...
      const insert = findCall(client.query, 'INSERT INTO quality_profiles');
      expect(insert[1][1]).toBe('Canonical BO');
      expect(insert[1][2]).toMatch(/Leading stock/);
    });
